...
export class AuthService {
  ...
  constructor(private restService: RestService, private router: Router, private poDialogService: PoDialogService) {
    this.userSubject = new BehaviorSubject<User>(
      JSON.parse(localStorage.getItem("@portfolio:user"))
    )
    this.user = this.userSubject.asObservable()
  }

  public get userValue(): User {
    const user = JSON.parse(localStorage.getItem("@portfolio:user"))
    return user
  }
      ...
      this.restService.post("/sessions", event).subscribe({
        next: (response: ResponseProps) => {
          ...
          if (!response.user.tfa || response.user.mustActiveTwoFactorAuthentication) {
            localStorage.setItem("@portfolio:user", JSON.stringify(valuesToSave))
          }
          ...
          if (response.user.isBlocked) {
            ...
            localStorage.removeItem("@portfolio:user")
            localStorage.removeItem("@portfolio:temp")
            reject()
          }
  ...
  get tablePreferences() {
    return JSON.parse(localStorage.getItem("@portfolio:preferences"))
  }
  ...
  browseTablePreferences(route: string, preferences: any) {
    ...
    localStorage.removeItem("@portfolio:preferences")
    localStorage.setItem("@portfolio:preferences", JSON.stringify(tablePreferences))
  } 
}
